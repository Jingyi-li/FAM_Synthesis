...
//	hls::stream<AXI_inter> r_in2[Phalf];
	...
	for (int loops = 0; loops<OUTLOOPS; loops++){
		Multi2One(r_out1,r_out2,r_AXI_out_value,r_AXI_out_al,r_AXI_out_fl);
	}
	...
	for(int i=0;i<132;i++){
		...
		AXI_label altt;
		AXI_label fltt;
		if (r_AXI_out_value.read_nb(valuett)){
			r_AXI_out_al.read_nb(altt);
			r_AXI_out_fl.read_nb(fltt);
			printf("temp[%d]: value=%f, al = %d, fl = %d, last = %d\n", i,valuett.value, altt.value,fltt.value,valuett.last);
		}
	}
	return 0;
}
