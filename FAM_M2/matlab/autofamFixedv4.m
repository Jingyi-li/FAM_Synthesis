function [Sx,alphao,fo,result]=autofamFixedv4(x,fs,df,dalpha,bitInput)
...
scal = (1-2^(-bit.firstFFT+2))/single(maxvalue);
result.Scale.FirstFFT = scal;
scal = fi(scal,1,bit.firstFFT+2,bit.firstFFT+1);
result.Scale.FirstFFTf = single(scal);
% XF1 = fi(XF1*scal,1,bit.ConjMulti,bit.ConjMulti-1);
...
scal = (1-2^(-bit.ConjMulti+2))/single(maxvalue);
result.Scale.Multi = scal;
scal = fi(scal,1,bit.ConjMulti+3,bit.ConjMulti+1);
% XM = fi(XM*scal,1,bit.secondFFT,bit.secondFFT-1);
XM = fi(XM*scal,1,bit.ConjMulti,bit.ConjMulti-1);
result.Multinormal = XM;
result.Scale.Multif = single(scal);
...
scal = (1-2^(-bit.secondFFT+2))/single(maxvalue);
result.Scale.SecondFFT = scal;
scal = fi(scal,1,bit.secondFFT+2,bit.secondFFT+1);
result.Scale.SecondFFTf = single(scal);
