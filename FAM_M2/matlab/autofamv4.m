function [Sx,alphao,fo,result]=autofamv4(x,fs,df,dalpha,bitInput,scale)
...
result.FirstFFTnormal = XF1.*scale.FirstFFTf;
XF1 = single(XF1.*scale.FirstFFTf);
%surf(log(abs(XF1)));
%title('FFT of data');
...
XM = single(XM);
result.Multinormal = XM.*scale.Multif;
XM = single(XM.*scale.Multif);
...
result.SecondFFTnormal = XF2.*scale.SecondFFTf;
XF2 = single(XF2.*scale.SecondFFTf);
