...
//Setup input and output AXI stream
	AXI_in_check stream_i_in, stream_q_in;
	hls::stream<AXI_in_check > i_in("i_in_stream");
	hls::stream<AXI_in_check > q_in("q_in_stream");
...
//	fclose(resultFile);
	...
	float avg_is = sum_is/65536/P;
	float avg_qs = sum_qs/65536/P;
	float avg_ir = sum_ir/65536/P;
	float avg_qr = sum_qr/65536/P;
