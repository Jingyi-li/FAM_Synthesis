...
//	}
//for(int i=0;i<10;i++){
//	a = (i*1.1/15.0)*(i*1.1/15.0);
//	in_data_t p = in_data_t(a);
//	out_data_t q = fxp_sqrt_top(p);
//	printf("a is %f, answer is %f, compute is %f\n",a,(i*1.1/15.0),q.to_float());
//}
	...
	FILE *resultFile = fopen("/home/jingyi/SCDfunctions/Paper/Synthesis_M1_SCD/src/matlab/deepsig_float_result_mag.dat","r");
	FILE *lastFile = fopen("/home/jingyi/SCDfunctions/Paper/Synthesis_M1_SCD/SCDlast.dat","w");
	printf("Save file \n");
	...
	for (int i = 0; i < OUTLOOPS/SIZE; i++) {
		for (int j = 0; j < Phalf; j++) {
			...
			single_fixed_t4 temp1 = (single_fixed_t4)rtemp1.data;
...
//			}
		...
		for (int j = 0; j < Phalf; j++) {
			...
			single_fixed_t4 temp2 = (single_fixed_t4)rtemp2.data;
			fscanf(resultFile,"%f,",&c);
