...
for idx_sig = 1
    ...
    % result_save.(['resultcell',num2str(index)]) = {'bits_w','bits_F1','bits_CM','bits_F2', 'normsimulation', 'normcalculation'};
        ...
        for l = 3:length(listing)
            if strfind(listing(l).name, [filename '.mat'])
                load([path '/' listing(l).name]);
                 result_temp = printResult_M1(result,resultF,bit,Type,ifdisp);
                exist = 1;
            end
        end
        ...
        if exist == 0
            switch Type
                ...
                case 'SSCA'
                    ...
                    [Sx,alphao,fo,result]=autosscaSingleNormv4_M1(x,fs,df,dalpha,bit);
            end
            %             save([path '/' filename '.mat'],'bit','result');
