function OutputSignal = FFTFixedv3_M1(InputSignal,bits)
...
%                 bit_new = bits+stage;
                ...
                b=fi(s(pos,:)-fi(fi(real(s(pos+Half,:)).*real(w),1,bit_pre,bit_frac )-...
                    fi(imag(s(pos+Half,:)).*imag(w),1,bit_pre,bit_frac)+...
                    (fi(imag(s(pos+Half,:)).*real(w),1,bit_pre,bit_frac )+...
                    fi(real(s(pos+Half,:)).*imag(w),1,bit_pre,bits-1))*1j,1,bit_pre,bit_frac),1,bit_new,bit_frac);
                s(pos,:)=bitsra(a,1);                                                  % saving computation of the 1-st part
                s(pos+Half,:)=bitsra(b,1);
            end
        end
        Half=Half*2;
    end
